...
- **`cloudflare/site.md`** (new): markdown representation of the page (mirrors `site/src/data/content.js`; sync rule noted — Track C copy changes update it too).
- **`cloudflare/wrangler.toml`** + **`cloudflare/README.md`** (new): routes `lumensweb.com/*` + `www.lumensweb.com/*`, deploy steps, DNS-AID record, DNSSEC-enable steps, live verification curl commands.
- **`cloudflare/test.mjs`** (new): `node test.mjs` → **31 checks green** against `worker-core.js` (Link headers, markdown + tokens, linkset+json shape, server card fields, WebMCP catalogue, health, full JSON-RPC round-trip, error codes). Pure Node, no Cloudflare runtime.
- **Deployed live**: DNS records proxied (orange-cloud ON) + `npx wrangler deploy` (version `d4702c12`). Verified live: `Server: cloudflare` + `CF-Ray` + `Link` headers on `/`; markdown returns `text/markdown` + `x-markdown-tokens: 431`; `api-catalog` → `application/linkset+json`; `tools/list` round-trip on `/mcp` works; `/health` ok. **isitagentready scan → level 4 "Agent-Integrated"** — linkHeaders ✅, markdownNegotiation ✅, apiCatalog ✅, mcpServerCard ✅, agentSkills ✅, webMcp ✅, robots/sitemap/signals ✅.
- **DNS-AID pending** (finding #2, the one remaining actionable fail): `_index._agents.lumensweb.com  SVCB 1 lumensweb.com alpn="mcp,h2" port=443 well-known="/.well-known/mcp/server-card.json" mandatory=alpn,port` (+ optional `_mcp._agents`) + DNSSEC-enable at registrar. `_a2a._agents` deliberately skipped — no A2A endpoint (no fabricated metadata).
- **Still N/A** (findings #5–7 + A2A card): OAuth/OIDC discovery, protected-resource, auth.md, A2A agent card — no protected APIs, no auth, no A2A agent; `/mcp` is public. Revisit only if a protected API/registration flow is added.
- Docs synced: `DEPLOYMENT.md` (status table — 4 items now ✅ Live; how-it-went-live + remaining DNS-AID step), `PROJECT-STRUCTURE.md` (cloudflare/ tree: worker.js + worker-core.js + rules), `ROADMAP.md` (recent-work entry), `claude.md` (file-tree row + build status). No ADR (no accent/font/CTA-wording/8-block changes; site/ untouched).
